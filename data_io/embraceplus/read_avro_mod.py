...
class ReadEmpaticaAvro():
    ...
    def get_tags(self, raw_dict, results_dict, key):
        """
        Get the tags timestamps from the avro file record.
        ...
        # raw tag data
        tags = ascontiguousarray(raw_dict["tagsTimeMicros"]) / 1e6

        results_dict[key] = {"tags": tags}



# tags_data = data["rawData"]["tags"]["tagsTimeMicros"]
  #          tags30min = pd.DataFrame(pd.to_datetime(tags_data, unit='us'),
   #                                   columns = ['tags'])
    #        tags = pd.concat([tags, tags30min])

   # tags["tags"] = tags["tags"] + pd.to_timedelta(timezone_offset, unit='s')
   # filename = export_folder + participant + "_tags.csv"
   # tags.to_csv(filename, index_label='time')
    ...
    def handle_resampling(self, streams):
        ...
        # Remove tags data stream
        tags_dict = streams.pop("tags")
